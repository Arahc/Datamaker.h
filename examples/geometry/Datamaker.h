...
FILE *startTest(std::string filename)
{
    return freopen(filename.c_str(), "wt", stdout);
}

void autoGenerate(std::string proname, std::vector<int> subtasks, void (*Generate)(int, int), std::string stdname = "", std::string valname = "")
{
    std::string stdrname = "./" + stdname;
    std::string valiname = "./" + valname;
    for (size_t i = 0; i < subtasks.size(); ++i)
        for (int j = 1; j <= subtasks[i]; ++j)
        {
            std::string filename = proname + (subtasks.size() == (size_t)1 ? "" : std::to_string(i + 1) + "-") + std::to_string(j);
            startTest(filename + ".in");
            Generate(i + 1, j);
            std::fflush(stdout);
            if (valiname != "./")
            {
                valname = valiname + " < " + filename + ".in";
            ...
            if (stdrname != "./")
            {
                stdname = stdrname + " < " + filename + ".in" + " > " + filename + ".out";
...
const struct Consts
{
    ...
    std::string SmallLetter = "abcdefghijklmnopqrstuvwxyz";
    std::string CapitalLetter = "ABCDEFGHIJKLMNOPQRSTUVWXTZ";
    std::string Letter = "AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxTyZz";
